package playtiLib.utils.load
{
	...
	/**
	 * Handles few loaders in the same time. Implements it by hash tables and by getting the object that need to be loaded, it's class
	 * represent it in the hash table and the event that need to be dispatched when it is complete loading. It also handles the progress of
	 * the loading.   
	...
	public class MultiLoadManager extends EventDispatcher{
		...
		//to be overriten incase of specific load senario
		public function load():void {}
		/**
		 * gets an instance of data that need to be loaded, it's class and an event type. It inserts the instance to hash in a cell with
		 * it's constractor name and listens to event(the event type that was sent). When it got this event, it take this instance from the
		 * hash and dispatch a COMPLETE event if it ready. It also listens to prograss event.
		 * @param instance
		 * @param constructor
		 * @param loadedEventType
		 * 
		 */		
		...
		/**
		 * Pulls the loaded object from the hash, checks if it ready and if it is, dispatches event(COMPLETE) 
		 * @param event
		 * 
		 */		
		...
		/**
		 * Dispatches progress Event with the bytes that already loaded. 
		 * @param event
		 * 
		 */		
		...
		/**
		 * Returns a uint number thet represent the progress of the byte loaded .  
		 * @return 
		 * 
		 */		
